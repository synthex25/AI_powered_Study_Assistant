...
async def _generate_with_fallback(
    ...
    try:
        ...
        # Only use Gemini billing fallback when DeepInfra is the configured provider.
        if (
            settings.LLM_PROVIDER.lower() == "deepinfra"
            and _is_billing_error(e)
            and settings.GOOGLE_API_KEY
        ):
            fallback = get_llm_provider(provider="gemini")
...
Keep it super brief (1-2 sentences). Be warm and human."""
        ...
        if _is_rate_limit_error(e):
            raise HTTPException(
                status_code=429,
                detail=(
                    "LLM rate limit reached. Please retry in a minute, "
                    "or switch provider/API key."
                ),
            )
        if _is_billing_error(e):
            raise HTTPException(
                status_code=402,
                detail=(
                    "Provider billing/quota issue. Add credits or "
