  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[1000] flex items-center justify-center bg-white/70 backdrop-blur-md"
      ...
      >
        ...
        >
          ...
          <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
            {/* Ambient Glow */}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <motion.div
                    ...
                    className="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 text-indigo-400 mb-6 shadow-lg shadow-indigo-500/10"
                  >
                    {step.icon}
                  </motion.div>
                  ...
                  <motion.p
                    ...
                    className="text-gray-400 max-w-md mx-auto mb-8 leading-relaxed"
                  >
                    {step.description}
                  </motion.p>
                    ...
                    >
                      {step.features.map((feature, index) => (
                        <motion.div
                          ...
                          className="flex items-center gap-2 bg-white/5 rounded-xl px-4 py-2.5 text-left border border-white/5"
                        >
                          <div className="w-5 h-5 rounded-full bg-indigo-500/20 flex items-center justify-center flex-shrink-0">
                            <FiCheck className="w-3 h-3 text-indigo-400" />
                          </div>
                ...
                <div className="flex items-center gap-3">
                  {!isFirstStep && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={handlePrev}
                      className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 font-medium transition-all border border-white/5"
                    >
                      <FiArrowLeft size={16} />
                      Back
                    </motion.button>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={handleNext}
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all"
                  >
                    {isLastStep ? (
                      <>
                        Get Started
