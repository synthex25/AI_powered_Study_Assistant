...
const Dashboard: FC = () => {
  ...
  const assistAI = async (files: File[]): Promise<void> => {
    try {
      ...
      console.log('[Dashboard] Upload response:', response.data);
      console.log('[Dashboard] Returned workspace_id:', response.data.workspace_id);
      console.log('[Dashboard] Documents:', response.data.documents);
      ...
      // Use workspace_id for chat queries
      const chatDocumentIds = [response.data.workspace_id || uploadWorkspaceId];
      console.log('[Dashboard] Setting documentIds for chat:', chatDocumentIds);

      // Set title from first document (use content snippet)
      const title = response.data.documents[0]?.processed_text[0]?.content?.substring(0,50) || 'Untitled';
      setAiTitle(title);

      // Update state with workspace_id
      setDocumentIds(chatDocumentIds);
  ...
  return (
    <div className="min-h-screen bg-white text-black overflow-hidden relative selection:bg-blue-200 font-sans">
      ...
      <div className="relative z-10 flex flex-col h-screen">
        ...
        <main className="flex-1 flex gap-6 p-6 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col gap-6 min-w-0">
                    ...
                    <motion.button
                       ...
                       className={`p-2.5 rounded-xl border transition-all ${
                        isSpeaking 
                          ...
                          : 'bg-white text-[#555555] border-[#e5e5e5] hover:bg-[#f5f5f5] hover:text-black'
                       }`}
            ...
            {/* Main Content Card */}
            <div className="flex-1 bg-white border border-[#e5e5e5] rounded-3xl overflow-hidden shadow-sm relative group">
               ...
               <AnimatePresence>
                 {(loading || aiLoading) && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 z-50 bg-white/90 flex flex-col items-center justify-center"
                  >
                      <div className="bg-white p-8 rounded-3xl border border-[#e5e5e5] shadow-sm flex flex-col items-center">
                      <Lottie
                      ...
                      />
                      <p className="text-[#2563eb] font-medium mt-4 animate-pulse">
              ...
              <div className="h-full overflow-y-auto custom-scrollbar">
                ...
                {activeTab === 'AI Notes' && (
                  aiNotesContent ? (
                    <div className="p-8">
                       <div
                        className="prose max-w-none prose-headings:text-black prose-a:text-[#2563eb] prose-strong:text-black"
                    ...
                    <EmptyState
                      icon={<FiEdit2 className="text-4xl text-blue-600" />}
                  ...
                  selectedFile ? (
                    <div className="h-full flex flex-col">
                      <div className="p-4 border-b border-[#e5e5e5] flex justify-between items-center bg-[#fafafa]">
                        <button
                          type="button"
                          onClick={goBack}
                          className="flex items-center gap-2 text-sm text-[#555555] hover:text-black transition px-3 py-1.5 rounded-lg hover:bg-[#f5f5f5]"
                        >
                          <FaArrowLeft /> Back to Files
                        </button>
                        <span className="text-sm font-medium text-black truncate max-w-[300px]">{selectedFile.name}</span>
                      </div>
                    ...
                    <div className="h-full p-8">
                      <div className="flex justify-between items-center mb-8">
                        <div>
                          <h3 className="text-2xl font-bold text-black">My Documents</h3>
                          <p className="text-[#555555] text-sm mt-1">Manage and process your uploaded PDFs</p>
                        </div>
                      ...
                      {uploadedFiles.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                          {uploadedFiles.map((file, index) => (
                            <motion.div
                              ...
                              key={`file-${index}`}
                              className="bg-white border border-[#e5e5e5] hover:bg-[#f5f5f5] hover:border-[#d4d4d4] rounded-2xl p-4 flex items-center gap-4 cursor-pointer transition-all group"
                            ...
                            >
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="font-medium text-black truncate group-hover:text-black transition-colors">
                                  {file.name}
                                </span>
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center text-center border-2 border-dashed border-[#e5e5e5] rounded-3xl bg-white m-4">
                           <motion.div 
                            whileHover={{ scale: 1.1, rotate: 5 }}
                            className="w-20 h-20 bg-[#f5f5f5] rounded-2xl flex items-center justify-center mb-6 ring-1 ring-[#e5e5e5] text-[#2563eb]"
                           >
                              <FaCloudUploadAlt className="text-4xl" />
                           </motion.div>
                          <h4 className="text-xl font-bold text-black mb-2">No documents yet</h4>
                          <p className="text-[#555555] max-w-xs mb-6">
