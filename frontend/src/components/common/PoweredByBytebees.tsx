...
const PoweredByBytebees: FC<PoweredByBytebeesProps> = ({ 
  ...
  return (
    <motion.div 
      className={`flex items-center ${currentSize.gap} ${currentSize.padding} rounded-2xl bg-white border border-blue-200 hover:bg-blue-50 hover:border-blue-300 transition-all cursor-pointer group shadow-sm ${className}`}
    ...
    >
      ...
      {showText && (
        <div className="flex flex-col justify-center border-l border-white/10 pl-3">
          <span className="text-[9px] text-blue-700 font-bold uppercase tracking-wider leading-none mb-1">Powered by</span>
          <span className={`${currentSize.text} font-black text-blue-900 leading-none`}>
            Bytebees
          </span>
        </div>
      )}
